...
export default function MenteeDashboard() {
  ...
  async function fetchTasks() {
    // Aufgaben holen, die nicht für diesen Mentee ausgeblendet sind
    ...
    const visible = (data ?? []).filter(t => !hiddenIds.includes(t.id))
    setTasks(visible)
  }
  ...
  async function handleLogSave(taskId, value) {
    ...
    if (existing) {
      ...
      await supabase
        .from('task_logs')
        .insert({ task_id: taskId, mentee_id: user.id, date: today, value })
    }
